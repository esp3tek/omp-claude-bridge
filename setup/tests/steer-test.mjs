...
let buf = ""; let text = ""; let steered = false; let done = false;
const t0 = Date.now();
...
child.stdout.on("data", (d) => {
	...
	while ((i = buf.indexOf("\n")) >= 0) {
		...
		if (o.type === "ready") {
			...
			if (!steered) { steered = true; setTimeout(() => { log("tool running → steer"); send({ id: "s1", type: "steer", message: "Palabra clave: GRANADA" }); }, 3000); }
			...
			if (parts) text = parts;
			if (o.type === "agent_end") { done = true; log(`agent_end → final text: ${JSON.stringify(text.slice(0, 200))}`); child.stdin.end(); }
		} else if (!steered && /tool/i.test(o.type)) {
			// unknown tool event shape: still schedule the steer
			steered = true; setTimeout(() => { log(`(${o.type}) tool running → steer`); send({ id: "s1", type: "steer", message: "Palabra clave: GRANADA" }); }, 3000);
		}
	}
});
